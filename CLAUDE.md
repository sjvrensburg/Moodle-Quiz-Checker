...
**The companion skills** (`skills/`): `moodle-quiz-tester` documents the validation workflow and `moodle-exams-quiz` the R/exams authoring workflow. `skills/moodle-quiz-tester/` is canonical — when changing CLI commands, HTTP endpoints, or report formats, update it (and README) in the same change, then re-sync `~/.claude/skills/moodle-quiz-tester/`. The repo's `moodle-exams-quiz` is a *scrubbed, generic* variant (module codes replaced with `MODULE` placeholders); the user's installed `~/.claude/skills/moodle-exams-quiz/` deliberately keeps their course-specific conventions — never overwrite it from the repo copy, and port only genuinely generic improvements between the two.
